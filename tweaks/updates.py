...
def apply_wu_mode(mode: str) -> tuple[bool, str]:
    ...
    # Pause uses PauseFeatureUpdatesStartTime/EndTime etc. Simplify via PS cmdlet when possible.
    ...
    if mode == "Auto download, schedule install":
        ...
        return (ok1 and ok2), f"{m1}; {m2}"
    if mode == "Pause for 7 days":
        return ps("Start-WUScan; Invoke-Command { Add-WUServiceManager -MicrosoftUpdate } ; Pause-WindowsUpdate -Days 7")
...
def apply_active_hours(start_h: int, end_h: int) -> tuple[bool, str]:
    if start_h == end_h:
        return False, "Active hours start and end cannot be the same"
    return ps(f"Set-ActiveHours -Start {start_h} -End {end_h}")
...
def get_tweaks() -> List[Tweak]:
    return [
        ...
        Tweak(
            ...
            apply=lambda v: (True, str(v))  # applied together with end
        ),
        Tweak(
            id="active_end",
            category="Windows Update",
